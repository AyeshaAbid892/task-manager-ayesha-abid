...
- **Tab switching:** `showTab(col)` in `ui.js` toggles `.active` on `.col-tab` buttons and `.tab-active` on `.kanban-col` elements. Badge counts on tabs are updated via `updateBadges()`.
...
TaskFlow uses a **unidirectional data flow** pattern without any framework:
